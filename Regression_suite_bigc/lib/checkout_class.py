...
class CheckoutClass(CheckoutData,CommonMethods ):
    ...
    def proceed_to_checkout(self, browser, url, country_data=None):
        element = self.wait_until_element_present('uniform-checkout_type_guest', "ID")
        element.click()
        # Checkout as Guest
        browser.find_element_by_id('CreateAccountButton').click()
        ...
        for item in country_data:
            if item=="Country" or item=="State":
                ...
                element=self.wait_until_element_present(country_data['State']['Element'], "CSS_SELECTOR", time=60)
                element.click()
            else:
    ...
    def account_details(self, account, email=None):
        ...
        try:
            if email!=None:
                element=self.find(account['Username']['Element'])
                element.clear()
                element.send_keys(email)
                self.find(account['Password']['Element']).send_keys(account['Password']['Value'])
            else:
                ...
                if item=="Select":
                    for subitem in payment_details[item]:
                        if subitem=="PaymentMethod":
                            self.select_dropdown_value_by_css(payment_details[item][subitem], payment_name)
                        else:
                            element=self.find_element_by_css_selector(payment_details[item][subitem])
                            element.click()
                else:
    ...
    def pay_for_order(self):
        try:
            ...
            self.wait_until_element_present('#test-order-number', "CSS_SELECTOR", time=60)
            assert "Your order was successfully received" in str(self.find_element_by_css_selector('.co-main--layout').text)
            orderID = str(self.find_element_by_css_selector('#test-order-number').text)
            return orderID[orderID.find(':')+1:]
        except NoSuchElementException:
            raise
    ...
    def continue_new_checkout(self, browser, shipping, shipping_address, payment='Authorize.net'):
        #self.account_details(browser, self.account_details_new)
