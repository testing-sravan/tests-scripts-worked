...
def test_edit_product(browser, url, email, password, state):
    ...
    e.send_keys(state['product_id'])
    try:
        browser.find_element_by_xpath('//span[@class="responsive-hide"]').click()
    except WebDriverException:
        e.send_keys(Keys.RETURN)
    common.wait_until_element_present("//tr[contains(.,'" + PRODUCT_NAME + "')]", 'XPATH', time=60)
    common.wait_until_element_present(PRODUCT_NAME, 'LINK').click()
